...
##
## model fitting
grouped1s<-svyglm(status~agegp+alcgp+tobgp,family=quasibinomial,design=des)
grouped2s<-svyglm(status~agegp+alcgp+tobgp+as.numeric(alcgp):as.numeric(tobgp),family=quasibinomial,design=des)
...
cont1s<-svyglm(case~age+I(age^2)+alcohol+tobacco,family=quasibinomial,design=tdes)
cont2s<-svyglm(case~age+I(age^2)+alcohol*tobacco,family=quasibinomial,design=tdes)
...
dotchart(c(SE(grouped1s)/SE(grouped1), NA, SE(cont1s)/SE(cont1))^2,
         pch=rep(c(1,19),c(12,6)),
