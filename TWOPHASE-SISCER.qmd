...
Want to fit `api00~enroll+ell+emer`

If we have `enroll, ell, emer` and `api99` we could try 

- raking on `api99` 
- raking on `ell`, `emer`, `enroll`
- raking on $\tilde h$ from `api99~enroll+ell+emer`
...
model0<-svyglm(api00~enroll+ell+emer, design=dstrat2ph)
```
...
dcal_pred <- calibrate(dstrat2ph, phase=2, 
                           formula=~enroll+ell+emer, calfun="raking")
model_y<-svyglm(api00~enroll+ell+emer, design=dcal_response)
model_x<-svyglm(api00~enroll+ell+emer, design=dcal_pred)
```
...
If phase 1 is iid or srs or has large strata, the best design-consistent estimator is of the AIPW class and has $\hat U=E[U_i\mid\text{phase 1}]$
...
Suppose we know from past years that 
...
optimum_allocation(df,strata="stratum",y="h",nsample=120)
```
