...
dvccc_aug<-twophase(id=list(~1,~1), strata=list(NULL,NULL),
  subset=~validated, data=mock.vccc)
dvccc_cal<-calibrate(dvccc_aug, formula=~0+`h.(Intercept)`+h.CD4_hat+h.Prior_ART,phase=2,calfun="raking")
```

- impute `ADE_val`, `CD4_val` in subsample
- predict to get `ADE_hat`, `CD4_hat` for everyone
- fit phase-1 model `ADE_hat~CD4_hat+Prior_ART`
- extract influence functions
